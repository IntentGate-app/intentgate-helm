...
- **Dependabot** alerts on every push, scanning repository config + GitHub Actions for known CVEs. Security updates are auto-proposed as PRs.
- Repository-specific lint and validation on every push and pull request, plus a weekly scheduled scan.
...
Chart packages publish to `oci://ghcr.io/intentgate-app/charts/` via signed GitHub Actions release workflows.
